...
if (isset($_POST['changeinfo'])) {
    $newName = mysqli_real_escape_string($con, $_POST['sername']);
    
    $customerId = $_SESSION['developer_id'];

    $updateQuery = "UPDATE developer SET sername = ? WHERE developer_id = ?";
...
if (isset($_POST['changeinfoo'])) {
    ...
    $newDateOfBirth = mysqli_real_escape_string($con, $_POST['ateofbirth']);
    
    $customerId = $_SESSION['developer_id'];

    $updateQuery = "UPDATE developer SET  ateofbirth = ? WHERE developer_id = ?";
    $stmt = mysqli_prepare($con, $updateQuery);
...
if (isset($_POST['changeinfooo'])) {
    ...
    $newPhoneNumber = mysqli_real_escape_string($con, $_POST['honenumber']);
    $customerId = $_SESSION['developer_id'];

    $updateQuery = "UPDATE developer SET  honenumber = ? WHERE developer_id = ?";
    $stmt = mysqli_prepare($con, $updateQuery);
...
$sql = "SELECT developer_id, mail, sername, ateofbirth,mage, honenumber FROM developer WHERE developer_id=?";
...
$result = mysqli_stmt_get_result($stmt);
    ...
    if ($result) {
        ...
        // For example, echo $customerDetails['sername'], $customerDetails['description'], etc.
        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <nav class="main-nav">
                        <!-- ***** Logo Start ***** -->
                        <a href="devhome.php" class="logo">
                            <img src="assets/images/logo.png" alt="" style="width: 158px;">
                        </a>
                        <!-- ***** Logo End ***** -->
                        <!-- ***** Menu Start ***** -->
                        <ul class="nav">
                            <li><a href="devhome.php" class="active">profile</a></li>
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
							...
							<div class="lform-group">
								...
								<div class="password-container">
									...
									include("../connect.php");
									$sid = $_SESSION['developer_id'];
									$sql = "SELECT developer_id, mail, honenumber, mage FROM developer WHERE developer_id=?";
									...
									?>
									<input type="file" id="mage" name="image">
								...
								if (isset($_POST['changeinf'])) {
									...
									if ($uploadOk == 0) {
										...
										if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
											...
											$updateQuery = "UPDATE developer SET mage = ? WHERE developer_id = ?";
											$stmt = mysqli_prepare($con, $updateQuery);
                            ...
                            <div class="lform-group">
                                <label for="sername"><font color="black">Name:</font></label>
                                <div class="password-container">
								
                                    <input type="text" id="sername" name="sername" placeholder="<?php echo isset($customerDetails) ? $customerDetails['sername'] : 'Enter new name'; ?>">
									<button type="submit" name="changeinfo" class="llogin-btn" >Change</button>
                                </div>
                            </div>

                            <div class="lform-group">
								<label for="ateofbirth"><font color="black">Date of Birth:</font></label>
								<div class="password-container">
									
									<input type="text" id="ateofbirth" name="ateofbirth" placeholder="<?php echo isset($customerDetails) ? $customerDetails['ateofbirth'] : 'Enter new date of birth'; ?>" pattern="\d{4}-\d{2}-\d{2}" title="Please enter a date in the format YYYY-MM-DD">
                            ...
                            <div class="lform-group">
                                <label for="honenumber"><font color="black">Phone Number:</font></label>
                                <div class="password-container">
                                    <input type="text" id="honenumber" name="honenumber" placeholder="<?php echo isset($customerDetails) ? $customerDetails['honenumber'] : 'Enter new phonenumber'; ?>">
