...
// Check the current number of rows in the admin table
$adminCountQuery = "SELECT COUNT(*) as count FROM admin";
$adminCountResult = mysqli_query($con, $adminCountQuery);
$adminCountData = mysqli_fetch_assoc($adminCountResult);
$currentAdminCount = $adminCountData['count'];

// Check if the registration limit is reached (2 or more rows)
if ($currentAdminCount >= 2) {
    echo "<script type='text/javascript'> alert('Registration limit reached. Cannot add more admins.')</script>";
} else {
    if (isset($_POST['Register'])) {
        ...
        $duplicateCheckQuery = "SELECT * FROM admin WHERE email = '$email' OR phonenumber = '$phonenumber'";
        ...
        if (mysqli_num_rows($duplicateCheckResult) > 0) {
            ...
            $admin_id = "Adm." . uniqid(); // Generate a unique ID for the admin
            $query = "INSERT INTO admin (admin_id, username, email, dateofbirth, phonenumber, password) VALUES ('$admin_id','$username', '$email', '$dateofbirth', '$phonenumber', '$password')";
    ...
    <div class="login-container">
        ...
        <form class="login-form" method="POST">
            <div class="role-options">
                <label>
                    <input type="radio" name="role" value="admin" onclick="redirectToRegistrationPage('admin')" checked>
