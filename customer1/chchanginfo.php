...
if (!isset($_SESSION['customer_login_status']) || $_SESSION['customer_login_status'] !== "logged in" || !isset($_SESSION['customer_id'])) {
...
if (isset($_POST['changeinfo'])) {
    $newName = mysqli_real_escape_string($con, $_POST['usernam']);
    
    $customerId = $_SESSION['customer_id'];

    $updateQuery = "UPDATE customer SET usernam = ? WHERE customer_id = ?";
...
if (isset($_POST['changeinfoo'])) {
    ...
    $newDateOfBirth = mysqli_real_escape_string($con, $_POST['dateofbirt']);
    
    $customerId = $_SESSION['customer_id'];

    $updateQuery = "UPDATE customer SET  dateofbirt = ? WHERE customer_id = ?";
    $stmt = mysqli_prepare($con, $updateQuery);
...
if (isset($_POST['changeinfooo'])) {
    ...
    $newPhoneNumber = mysqli_real_escape_string($con, $_POST['phonenumbe']);
    $customerId = $_SESSION['customer_id'];

    $updateQuery = "UPDATE customer SET  phonenumbe = ? WHERE customer_id = ?";
    $stmt = mysqli_prepare($con, $updateQuery);
                ...
                <div class="login-containert">
                    <div>
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
								...
								if (isset($_POST['changeinf'])) {
									...
									if ($uploadOk == 0) {
										...
										if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
											...
											$updateQuery = "UPDATE customer SET imag = ? WHERE customer_id = ?";
											$stmt = mysqli_prepare($con, $updateQuery);
                            ...
                            <div class="lform-group">
                                <label for="usernam"><font color="black">Name:</font></label>
                                <div class="password-container">
								
                                    <input type="text" id="usernam" name="usernam" placeholder="<?php echo isset($customerDetails) ? $customerDetails['usernam'] : 'Enter new name'; ?>">
									<button type="submit" name="changeinfo" class="llogin-btn" >Change</button>
                                </div>
                            </div>

                            <div class="lform-group">
								<label for="dateofbirt"><font color="black">Date of Birth:</font></label>
								<div class="password-container">
									
									<input type="text" id="dateofbirt" name="dateofbirt" placeholder="<?php echo isset($customerDetails) ? $customerDetails['dateofbirt'] : 'Enter new date of birth'; ?>" pattern="\d{4}-\d{2}-\d{2}" title="Please enter a date in the format YYYY-MM-DD">
                            ...
                            <div class="lform-group">
                                <label for="phonenumbe"><font color="black">Phone Number:</font></label>
                                <div class="password-container">
                                    <input type="text" id="phonenumbe" name="phonenumbe" placeholder="<?php echo isset($customerDetails) ? $customerDetails['phonenumbe'] : 'Enter new phonenumber'; ?>">
