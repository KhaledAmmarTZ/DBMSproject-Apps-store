...
<body>
    ...
    <div class="single-product section">	
		...
		$transactionSql = "SELECT r.report_id, r.additionalComments, r.bug, r.crash, r.performance, r.feature, p.image AS product_image, p.app_name, p.version
									   FROM report r
		...
		// Display report and product information
					while ($rowreport = mysqli_fetch_assoc($resultTransaction)) {
						echo'<div class="container">';
						echo'<div class="row">';
						echo'<div class="col-lg-6">';
						echo '<div class="left-image">';
						// Display product image
						$productImage = "../image/" . $rowreport['product_image'];
						...
						echo '<h4>Report ID: ' . $rowreport['report_id'] . '</h4>';
						echo '<p>Report Comment: ' . $rowreport['additionalComments'] . '</p>';
						echo '<p>App Name: ' . $rowreport['app_name'] . '</p>';
						$versions = array();

						// Check each attribute and add corresponding version to the array
						if ($rowreport['bug'] == 1) {
							$versions[] = 'Bug';
						}
						if ($rowreport['crash'] == 1) {
							$versions[] = 'Crash';
						}
						if ($rowreport['performance'] == 1) {
							$versions[] = 'Performance';
						}
						if ($rowreport['feature'] == 1) {
							$versions[] = 'Feature';
						}

						// Display versions or "NONE" if all attributes have 0
						if (count($versions) > 0) {
							echo '<p>Report Cause: ' . implode(', ', $versions) . '</p>';
						} else {
							echo '<p>Report Cause: NONE</p>';
						}
						echo '</div>';
